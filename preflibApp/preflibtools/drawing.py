...
def drawGraph(instance, imgFilePath, maxNumNode = 100, isWMD = False):
	print(f"Drawing: {str(imgFilePath)}")
	startTime = time.time()
	plt.close('all')
	nxGraph = nx.DiGraph()
	nxGraph.add_nodes_from([n for n in instance.graph.dict if n <= maxNumNode])
	print(
		f"{len(nxGraph.nodes())} nodes added to nxGraph: {str(time.time() - startTime)}"
	)
	...
	print(
		f"{len(nxGraph.edges())} edges added to nxGraph: {str(time.time() - startTime)}"
	)


	node_color = [
		sum(e[2] for e in instance.graph.outgoingEdges(n) if e[1] <= maxNumNode)
		for n in nxGraph.nodes()
	]

	print(f"Nodes color computed: {str(time.time() - startTime)}")

	layout = nx.drawing.layout.kamada_kawai_layout(nxGraph) if isWMD else nx.drawing.layout.circular_layout(nxGraph)
	print(f"Layout computed: {str(time.time() - startTime)}")
	...
	plt.axis('off')
	print(f"Figure generated: {str(time.time() - startTime)}")
	plt.savefig(imgFilePath)
	print(f"Finished: {str(time.time() - startTime)}")
	plt.close('all')
